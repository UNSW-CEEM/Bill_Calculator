...
def bill_calculator(load_profile, tariff, network_load=None, FiT=True):
    ...
    Results = {}
    t0 = time.time()
    load_profile_imp = load_profile.copy().clip(lower=0)
    imports = [np.nansum(load_profile_imp[col].values)
    ...
    if FiT:
        exports = [np.nansum(load_profile[col].values[load_profile[col].values < 0])
               for col in load_profile.columns]
        Results['LoadInfo']['Annual_kWh_exp'] = [-1 * x for x in exports]
    ...
    for TarComp, TarCompVal in tariff['Parameters'].items():
        if 'Daily' in TarCompVal.keys():
            Results[TarComp]['DailyCharge'] = num_days * TarCompVal['Daily']['Value']

    # Energy


    # Check if volumetric charge exists
    for TarComp, TarCompVal in tariff['Parameters'].items():
        print(TarCompVal)
        if 'FlatRate' in TarCompVal.keys():
            if TarCompVal['Daily']['Value'] > 0:
                fr_flag = True
                break
